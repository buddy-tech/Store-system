...
namespace Store_System.PL
{
    public partial class FRM_ADDpayedCOM : Form
    {
        ...
        public void load_all_PAYEDmoney_COM()
        {
            ...
            ds = cls.LOAD_ALL_paytedmoneyCOM_todgv();

            dataGridView1.DataSource = ds;
            lb_NUM_CUSTOMER.Text = "عدد المدفوعات للموردين :" + " " + dataGridView1.RowCount.ToString();
        }
        public void load_all_COMName()
        {
        ...
        private void FRM_ADDpayedCOM_Load(object sender, EventArgs e)
        {
            load_all_COMName();
        }
        ...
        private void btn_search_cut_Click(object sender, EventArgs e)
        {
            ...
            if (cmbCUT_Name.Text != "")
            {
                ...
                dt = cls.SEARCH_AllCOM_NAME(cmbCUT_Name.SelectedItem.ToString());
                dataGridView1.DataSource = dt;
                lb_NUM_CUSTOMER.Text = "عددالمدفوعات للموردين:" + " " + dataGridView1.RowCount.ToString();
            }
            else
            {
                load_all_PAYEDmoney_COM();
        ...
        {
            if (MessageBox.Show("هل انت متاكد من تعديل معلومات المدفوعات للموردين؟", "عملية التعديل", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                ...
                frm.lb_NAME.Text = "اسم المورد";
                frm.Text = "تعديل معلومات المدفوع للمورد " + this.dataGridView1.CurrentRow.Cells[1].Value.ToString();
        ...
        private void حذفToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("هل انت متاكد من حذف المدفوعات للمورد؟", "عملية الحذف", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                cls.Delete_Payedmoney_com(Convert.ToInt32(this.dataGridView1.CurrentRow.Cells[0].Value));
                load_all_PAYEDmoney_COM();
