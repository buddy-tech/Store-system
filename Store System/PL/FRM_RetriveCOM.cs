...
{
    public partial class FRM_RetriveCOM : Form
    {
        ...
        public FRM_RetriveCOM()
        {
            InitializeComponent();
            dataGridView1.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
        }

        private void FRM_RetriveCOM_Load(object sender, EventArgs e)
        {
        ...
        private void btn_SearchAll_Orders_Click(object sender, EventArgs e)
        {
            ...
            dt = cls.Load_AllOrderDetailsCOM_SOrderID(Convert.ToInt32(frm.Order_ID));
        ...
        private void btn_AddOrder_Click(object sender, EventArgs e)
        {
            ...
            if (exist == 0)
            {
                if (state == "add")
                {
                    ...
                    MessageBox.Show(lb_nameProduct.Text + " " + "تم بنجاح اضافة كمية من الصنف", "اضافة كمية من صنف", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    dt = cls.Load_AllOrderDetailsCOM_SOrderID(Convert.ToInt32(tb_ORDER_ID.Text));
                    dataGridView1.DataSource = dt;
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        overAll_total += Convert.ToDecimal(dataGridView1.Rows[i].Cells[5].Value);
                    lb_overALLtotal.Text = overAll_total.ToString();
