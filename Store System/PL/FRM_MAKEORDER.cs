...
{
    public partial class FRM_MAKEORDER : Form
    {
        DataTable dt;
        BL.CLS_MakeORDER cls = new BL.CLS_MakeORDER();
        frm_AllProduct_IN_sALLSTORES frm = new frm_AllProduct_IN_sALLSTORES();
        BL.CSL_load_allORERS cls2 = new BL.CSL_load_allORERS();

        public string state = "add";
        public string Old_QTY;
        public int counter = 0;

        public FRM_MAKEORDER()
        {
            InitializeComponent();
            dataGridView1.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
        }

        private void FRM_MAKEORDER_Load(object sender, EventArgs e)
        {
            tb_usertypeName.Text = "mos";//Form_LOGIN.get_MainForm.usertype_name
            tb_ORDER_ID.Text = cls.fillID();
        ...
        {
            MANG_FRM_customers frm = new MANG_FRM_customers();
            frm.ShowDialog();
        ...
        {        
            ...
            if (exist == 0)
            {
                if (state == "add")
                {
                    ...
                    if (counter == 0)
                        cls.ADD_Order(Convert.ToDateTime(dateTimePicker1.Text), Convert.ToInt32(lb_CustomerID.Text), OTID,usertype_ID);
                    counter++;
        ...
        private void حذفToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("هل انت متاكد من حذف هذا الصنف من الفاتورة؟", "عملية الحذف", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                ...
                cls.DELETE_PinOrderDetails(Convert.ToInt32(this.dataGridView1.CurrentRow.Cells[0].Value));
       ...
       public DataTable LoadAllorders()
        {
            DataTable ds = new DataTable();
                ds = cls2.LOAD_ALL_Orders_todgv();
               return ds;
        }
        private void btn_PRTOrder_Click(object sender, EventArgs e)
        {
            MessageBox.Show("تم حفظ الفاتورة بنجاح", "عملية حفظ الفاتورة", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            //cls.save_OverAlltotalORDER(Convert.ToInt32(tb_ORDER_ID.Text), Convert.ToDecimal(lb_overALLtotal.Text));
            RPT.Form1 frm = new RPT.Form1();
             int orderID = Convert.ToInt32(cls.fillIDOrderForPrint());
            RPT.CrystalReport1 rpt = new RPT.CrystalReport1();
            rpt.SetDataSource(cls.Load_Order_ForPrint(orderID));
